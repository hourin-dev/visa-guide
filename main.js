 ...
 * 담당: 이규희 사무장
 * 업데이트: 1순위/2순위 복수 추천 및 리포트 가독성 최적화
 */
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 3. 법무부 출입국 정책 분석 및 2-WAY 리포트 생성 로직
    document.getElementById('run-btn').addEventListener('click', async () => {
        ...
        // AI 리포트 생성 프롬프트 (2가지 추천안 및 사무장님 양식 적용)
        const reportPrompt = `
        ...
        * 🥈 **[제2순위] 추천비자** : (비자명) - [ <span class="red-text">자격취득가능 : OO%</span> ]
        ...
        ### 🌟 **4. 취득 시 주요 장점**
        * **제1순위 비자 취득 시**: (핵심 장점 2가지)
        * **제2순위 비자 취득 시**: (핵심 장점 2가지)

        ### 📊 **5. 분석 및 점수 계산**
        (지침서 기준 항목별 예상 점수 배점을 목록 형태로 정리)
