document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('run-btn').addEventListener('click', async () => {
        ...
        btn.disabled = true;
        // 요청하신 문구로 변경 및 모래시계 추가
        btn.innerHTML = '<span class="loading-icon">⏳</span> 법무부 출입국 정책분석 중 ...';
        ...
        try {
            ...
            for(let model of models) {
                log(`📡 [Search] ${model.name.split('/')[1]} 모델 연결 시도...`);
                try {
                        ...
                        body: JSON.stringify({
                            contents: [{ parts: [
                                { text: `당신은 법무법인 대림의 이규희 사무장입니다. 
                                        ...
                                        작성일: 2025년 12월 22일. 분석대상: E-7-4, E-7-R, F-2-R, F-2-7, F-4 전체.
                    ...
                    if (resData.candidates) {
                        ...
                        document.getElementById('result-content').innerHTML = resData.candidates[0].content.parts[0].text.replace(/\n/g, '<br>');
                        log(`✅ 분석 완료! (종료 시간: ${new Date().toLocaleTimeString()})`);
                        break;
                    }
                } catch(e) { continue; }
            }
